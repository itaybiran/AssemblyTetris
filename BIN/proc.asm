...
proc YesOrCancel
	...
	checkAgain:
		...
		call mouse
		call menu_ ; אם כן חוזר לתפרעט הראשי
	ret
	nextButtonCheckYes:
		call yes_button
		cmp [check_pressed], 1 ;בודק אם נלחץ כפתור הביטול
...
proc CrazyBoard_Mood
	...
	call clearkeyboardbuffer
	mov [Score], 0 											; מאפס את ניקוד השחקן
	mov [Speed], 130											; מאפס את המהירות למהירות ממוצעת
	call initializeClock ;מאפס את השעון
	call BoardDrawer 									; מצייר את הלוח
	push 47h
	call Draw_Middle_Oblong 								; מצייר את המלבן העליון באמצע המסך
...
proc NoRorations_Mood
	...
	push 0FCh
	call Draw_Middle_Oblong 								; מצייר את המלבן העליון באמצע המסך
	mov [NoRorations_check], 1 							; התחיל המשחק של השחור לבן
	...
	End_NoRorations:
	mov [NoRorations_check], 0							; נגמר המשחק של השחור לבן
	call updateBest ; בודק אם המשתמש שבר את השיא שלו
	ret
...
proc Israel_Mood
	...
	call clearkeyboardbuffer
	mov [Score], 0 											; מאפס את ניקוד השחקן
	mov [Speed], 80											; מאפס את המהירות למהירות ממוצעת
	call initializeClock ;מאפס את השעון	
	call BoardDrawer 										; מצייר את הלוח
	push 0FCh
	call Draw_Middle_Oblong 								; מצייר את המלבן העליון באמצע המסך
	...
	Israel:											; לולאת המשחק
			...
			call GetRandomCube 								;מגריל קוביה אקראית
			call changeToWhiteOrBlue 						;change the color of the cube to black or white randomly
			call cube_Starts							 	; קוביה יורדת ופועלת כמו שצריך עד שנעצרת בנחיתה על משטח
			call destroyLinesCheck 							; בודק אם המשתמש השלים שורה ופועל בהתאם
			call endGameCheck  								; בודק אם המשתמש הפסיד וקוביה נגעה בחלקו העליון של הלוח
			cmp dx, 1 
	...
	End_Israel:
	mov [Israel_check], 0							; נגמר המשחק של השחור לבן
	call updateBest ; בודק אם המשתמש שבר את השיא שלו
	ret
...
proc cube_Starts
		...
		cube_:
			...
			moveCube: ; move down the cube
				call ScoreBoardClock ; מדפיס את הזמן של השעון
...
proc Movment_Cube
	...
	key:
		...
		cmp [k],'w' ;up
		je rotate
	 ; בודק את התו שקיבלנו ופועל בהתאם
...
proc RotateCube
		...
		cmp [Israel_check], 1 ;בודק אם המוד שהשחקן משחק בו הוא שחור לבן או לא
